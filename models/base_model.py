...
class BaseModel:
    ...
    def build_network_timm(self, net_opt):
        set_opt = net_opt["setting"]
        num_classes = set_opt["num_classes"]
        pretrained = set_opt["pretrained"] if "pretrained" in set_opt else False
        path = set_opt["path"] if "path" in set_opt else ""
        net = timm.create_model(net_opt["which_network"], pretrained=pretrained, checkpoint_path=path, num_classes=num_classes)
        net = self.model_to_device(net)
        self.print_network(net)
        return net
