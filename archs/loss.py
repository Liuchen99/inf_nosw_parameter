...
class KDLoss(nn.Module):
    ...
    def forward(self, res, ref, temperature):
        log_pred = F.log_softmax(res / temperature, dim=1)
        soft_target = F.softmax(ref / temperature, dim=1)
        loss = F.kl_div(log_pred, soft_target, reduction="none").sum(1).mean()
        loss *= temperature ** 2
        return loss
...
class KLLoss(nn.Module):
    ...
    def forward(self, res, ref):
        return F.kl_div(res, ref, reduce="sum")
...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, res, ref):
        return F.cross_entropy(res, ref)
