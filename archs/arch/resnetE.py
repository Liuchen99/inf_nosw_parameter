...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride != 1:
            if in_planes == BasicBlock.expansion * planes:
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000, in_planes=64):
        ...
        self.layer1 = self._make_layer(block, in_planes * 1, num_blocks[0], stride=2, layer_idx=1)      # out: 56x56
...
class ResNetE18L(ResNet):
    def __init__(self, num_classes):
        super().__init__(BasicBlock, [2, 2, 2, 2], num_classes, in_planes=64)


@ARCH_REGISTRY.register()
class ResNetE18S(ResNet):
    def __init__(self, num_classes):
        super().__init__(BasicBlock, [2, 2, 2, 2], num_classes, in_planes=16)


@ARCH_REGISTRY.register()
class ResNetE18SA(ResNet):
    def __init__(self, num_classes):
        super().__init__(BasicBlock, [1, 2, 3, 2], num_classes, in_planes=16)
...
if __name__ == "__main__":
    ...
    test(ResNetE18L(1000))
    test(ResNetE18S(1000))
